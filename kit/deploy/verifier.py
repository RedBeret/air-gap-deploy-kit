...
def check_acme_api(
    ...
    *,
    ...
    """
    Ping acme-parts-cloud: GET /admin/healthz, with /health as a legacy fallback.
    Returns ok if HTTP 200 with {"status": "ok"}.
    """
    paths = ("/admin/healthz", "/health")
    errors = []
    try:
        for path in paths:
            if client is not None:
                resp = client.get(f"{base_url}{path}", timeout=5)
            else:
            ...
            if data.get("status") == "ok":
                return VerifyResult("acme-parts-cloud", ok=True, detail=f"GET {path} -> 200 ok")
            return VerifyResult(
                "acme-parts-cloud",
                ok=False,
                detail=f"GET {path} returned unexpected body: {data}",
            )
        return VerifyResult("acme-parts-cloud", ok=False, detail="; ".join(errors))
...
def check_ollama(
    ...
    *,
    ...
    try:
        ...
        if ":" in model:
            matched = [p for p in pulled if p == model or p.startswith(f"{model}-")]
        else:
            matched = [p for p in pulled if p.split(":")[0] == model]
        if matched:
            return VerifyResult(
                "ollama",
                ok=True,
