...
def test_build_file_checksums_skips_manifest():
    with tempfile.TemporaryDirectory() as tmp:
        bundle_dir = Path(tmp)
        (bundle_dir / "wheels").mkdir()
        (bundle_dir / "wheels" / "demo.whl").write_bytes(b"wheel")
        (bundle_dir / "manifest.json").write_text("{}")

        checksums = build_file_checksums(bundle_dir)

        assert "wheels/demo.whl" in checksums
        assert "manifest.json" not in checksums


def test_verify_file_checksums_detects_tamper():
    with tempfile.TemporaryDirectory() as tmp:
        bundle_dir = Path(tmp)
        payload = bundle_dir / "docker" / "image.tar"
        ...
        manifest = BundleManifest(
            ...
            created_at="2025-01-01T00:00:00Z",
            platform="x86_64",
            file_checksums=build_file_checksums(bundle_dir),
        )

        payload.write_bytes(b"tampered")
        errors = verify_file_checksums(manifest, bundle_dir)
