...
def test_check_acme_api_ok(httpx_mock: HTTPXMock):
    httpx_mock.add_response(url="http://localhost:8000/admin/healthz", json={"status": "ok"})
...
def test_check_acme_api_wrong_body(httpx_mock: HTTPXMock):
    httpx_mock.add_response(url="http://localhost:8000/admin/healthz", json={"status": "degraded"})
    with httpx.Client() as client:
        result = check_acme_api("http://localhost:8000", client=client)
...
def test_check_ollama_does_not_match_different_tag_prefix(httpx_mock: HTTPXMock):
    httpx_mock.add_response(
        url="http://localhost:11434/api/tags",
